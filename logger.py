...
class Logger:
    ...
    def save_cpk(self, emergent=False):
        ...
        basename = 'checkpoint_' + self.train_mode + '.pth'
...
class Visualizer:
    ...
    def visualize(self, driving, source, out):
        ...
        # bg
        if 'bg' in out:
            bg = out['bg'].data.cpu().numpy()
            bg = np.transpose(bg, [0, 2, 3, 1])
            images.append(bg)
        
        # mask_pred
        if 'mask_pred' in out:
            mask_pred = out['mask_pred'].data.cpu().repeat(1, 3, 1, 1).numpy()
            mask_pred = np.transpose(mask_pred, [0, 2, 3, 1])
            images.append(mask_pred)
        ...
        #     for i in range(out['deformed_source'].size(1)):
        #         deformed_source = out['deformed_source'][:, i].data.cpu().numpy()
        #         deformed_source = np.transpose(deformed_source, [0, 2, 3, 1])
        #         images.append(deformed_source)
        if 'optical_flow' in out:
            identity_grid = make_coordinate_grid((out['optical_flow'].size(-3), out['optical_flow'].size(-2)), type=out['optical_flow'].type())
            denseflow = ((out['optical_flow'] - identity_grid)*out['optical_flow'].size(-2)).data.cpu().numpy()
        ...
        # if 'feat' in out['driving_region_params']:
        #     for i in range(out['driving_region_params']['feat'].size(1)):
        #         heatmap = F.interpolate(out['driving_region_params']['feat'][:,i:i+1,:,:], size=source.shape[1:3])
        #         heatmap = np.transpose(heatmap.data.cpu().numpy(), [0, 2, 3, 1])
        #         images.append(draw_colored_heatmap(heatmap, self.colormap, self.region_bg_color))
    
        # if 'feat' in out['source_region_params']:
        #     for i in range(out['source_region_params']['feat'].size(1)):
        #         heatmap = F.interpolate(out['source_region_params']['feat'][:,i:i+1,:,:], size=source.shape[1:3])
        #         heatmap = np.transpose(heatmap.data.cpu().numpy(), [0, 2, 3, 1])
        #         images.append(draw_colored_heatmap(heatmap, self.colormap, self.region_bg_color))
